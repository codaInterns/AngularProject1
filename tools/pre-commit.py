...
def fun():	
	...
	for i in diff:
		...
		if os.path.splitext(base)[1] == '.ts':
			...
			errors = cmd
			cmd = cmd.splitlines()
			if cmd.count('Starting audit...') != 0:
				cmd.remove('Starting audit...')
			if cmd.count('Audit done.') != 0:
				cmd.remove('Audit done.')
			if cmd.count('Files to process must be specified, found 0.') != 0:
				cmd.remove('Files to process must be specified, found 0.')	
			if len(cmd) != 0:
				isLintingProper = 0
